  ...
  if (!client) {
    ...
    if (!exists) {
      ...
      await db.collection("users").insertOne({
        username: "admin", password: hash, expire_date: expire, credits: 999
      });
    }
  }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return respond(res, { success: false, message: "فشل الاتصال بقاعدة البيانات" });
  }
  ...
  if (action === "deduct_credit") {
    ...
    if (user.credits < price)
      return respond(res, { success: false, message: "رصيدك غير كافٍ", credits: user.credits });
  ...
  if (action === "create_user") {
    if (!await isAdmin(db, input.admin_token))
    ...
    if (exists) return respond(res, { success: false, message: "اسم المستخدم موجود مسبقاً" });
    ...
    return respond(res, { success: true, message: `تم إنشاء ${username} بصلاحية ${days} يوم ورصيد ${credits}` });
  }
  ...
  if (action === "add_credits") {
    ...
    await db.collection("users").updateOne({ username }, { $inc: { credits: amt } });
    return respond(res, { success: true, message: `تم إضافة ${amt} كريدت لـ ${username}`, credits: user.credits + amt });
  }
  ...
  if (action === "deduct_credits_admin") {
    ...
    await db.collection("users").updateOne({ username }, { $inc: { credits: -amt } });
    return respond(res, { success: true, message: `تم خصم ${amt} من ${username}`, credits: user.credits - amt });
  }
  ...
  if (action === "delete_user") {
    ...
    return respond(res, { success: true, message: "تم حذف " + input.username });
  }
  ...
  if (action === "list_users") {
    ...
    return respond(res, { success: true, users: result });
  }
